...
- (void)onReset {
    ...
    CGFloat barMargin = 1;
    CGFloat barWidth = floorf((width - barMargin * (kBarCount + 1)) / kBarCount); // 取不大于本身的最大整数
    CGFloat barOrginX = roundf((width - (barMargin + barWidth) * kBarCount + barMargin) / 2.0); // 取本身的四舍五入
...
- (void)onSort {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        switch (weakSelf.segmentControl.selectedSegmentIndex) {
            case 0:
                [weakSelf selectionSort];
                break;
            case 1:
                [weakSelf bubbleSort];
                break;
            case 2:
                [weakSelf insertionSort];
                break;
            case 3:
                [weakSelf quickSort];
                break;
